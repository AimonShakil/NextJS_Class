  ...
  return (
    <main
      className={`w-screen h-screen ${
        status === "pending" ? "bg-blend-overlay, opacity-50" : "opacity-100"
      }`}
    >
      <div className="flex flex-col gap-y-4">
        ...
        <div className="flex justify-center items-center gap-x-2">
          ...
          <button
            onClick={() => {
              dispatch(addAsync(input));
            }}
            disabled={status === "pending"}
            className="bg-green-700 text-sm rounded px-3 py-1 text-slate-100"
          >
            Add Async
          </button>
          <button
